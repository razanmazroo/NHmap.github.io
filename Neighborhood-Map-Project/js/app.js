...
function MapViewModel() {
    ...
    self.filterdItems = ko.computed(function() {
        ...
        if (!filter) {
            ...
            return ko.utils.arrayFilter(self.locationItems(), function(item) {
                item.visible(item.title.toLowerCase().search(filter));
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('.weather').find('.weather_date').text(data.query.results.channel[0].item.forecast.date);
            $('.weather').find('.weather_temp_min').text(data.query.results.channel[0].item.forecast.low + data.query.results.channel[0].units.temperature);
            $('.weather').find('.weather_temp_max').text(data.query.results.channel[0].item.forecast.high + data.query.results.channel[0].units.temperature);
            $('.weather').find('.weather_icon').attr('src', iconUrl + data.query.results.channel[0].item.forecast.code + '.gif');
            $('.weather').find('.weather_text').text(data.query.results.channel[0].item.forecast.text);
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            self.venueContentString =
                ...
                '<div>' + data.response.venues[0].name + '</div>' +
                '<div>' + data.response.venues[0].location.formattedAddress[0] + "</div>" +
                '<div>' + data.response.venues[0].location.formattedAddress[1] + "</div>";
        },
